...
- As a fail-closed backstop, the boot configurator also writes a small fixed set of clearly-destructive shell commands (`ha core stop`, `ha host reboot`/`shutdown`, `ha os update`, `ha supervisor stop`/`restart`) into `permissions.deny`, which Claude Code enforces even if the hook never runs. This floor is shell-only: it does not back up the MCP `call_service_tool` path, because permission rules cannot match a tool's arguments, so that path relies on the hook (`jq` is required for it). Reading verbs like `ha core logs` and `ha core info` are deliberately not in the floor.
